...
const manifestPath = resolve(ROOT, 'data/visuals/dining-table-pbr-assets.v1.json');
const REQUIRED_ITEMS = Object.freeze(['table-001', 'desk-001', 'table-003']);
...
describe('PROD-014 dining/table PBR asset manifest', () => {
  it('owns every remaining dining, writing and computer table through a versioned bounded PBR manifest', () => {
    const manifest = JSON.parse(readFileSync(manifestPath, 'utf8'));

    expect(manifest.schemaVersion).toBe(1);
    expect(manifest.packId).toBe('dining-table-pbr-v1');
    expect(manifest.bakeWorkflow).toBe('high-to-low-normal-plus-material-passes');
    expect(manifest.maxPackBytes).toBeLessThanOrEqual(3_600_000);
    ...
    for (const asset of manifest.assets) {
      ...
      expect(asset.path).toMatch(/^\/assets\/furniture\/dining\/.+\.glb$/);
      expect(asset.maxBytes).toBeLessThanOrEqual(1_400_000);
    }
  });
