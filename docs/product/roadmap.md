...
| P1 | PBR storage asset pack | Каталоговые полки, шкафы, комоды и тумбы получают distinct authored PBR silhouettes вместо procedural fallback. | Отдельный versioned manifest, red content contract, reproducible Blender source, UV1/ORM validation, measured budget and game-camera smoke; без изменения storage score или persistence. |
