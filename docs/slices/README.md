...
Этот каталог хранит evidence завершённых production vertical slices. Отчёт фиксирует пользовательский результат, versioned contracts, architecture boundaries, verification evidence и explicit non-goals. Он не заменяет active product, architecture or content references, которые остаются в [documentation hub](../README.md).
...
| [PROD-023 — Client-priority and multi-style evaluator](PROD-023-client-priority-multi-style-evaluator.md) | Completed | ClientBrief V2 activates exact multi-style targets, personal priority rules and spatial preferences through a deterministic three-channel score and explanation V2. |
| [PROD-024 — Semantic catalog coverage](PROD-024-semantic-catalog-coverage.md) | Completed | Every shipped item now declares explicit functional and spatial behavior; only authored floor obstacles enter occupancy and generic clearance. |
| [DOC-HOTFIX-003 — Project truth consolidation](DOC-HOTFIX-003-project-truth-consolidation.md) | Completed | Root public entry point, changelog and data lifecycle guide now distinguish active V4/V2 runtime truth from retained historical contracts. |
