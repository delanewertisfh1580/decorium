...
Контент редактируется без изменения Domain-кода и поставляется вместе со статическим приложением. MVP не требует filesystem API, backend, database или environment variables. Схемы и loaders должны обновляться вместе с контрактом V2; legacy 8-field format не является runtime-контрактом.
