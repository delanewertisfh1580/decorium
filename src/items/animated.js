// =============================================================================
// items/animated.js — живые пропы: ТВ (CanvasTexture), кот/собака (патруль по
// CatmullRomCurve3), торшер (PointLight), растение (покачивание).
// Пропы живут ВНЕ manager: не участвуют в drag/raycast/оценке.
// Вся расстановка — seeded (Rng), без Math.random().
// =============================================================================
...
// --- ТВ: корпус + тумба + экран (CanvasTexture), смена «канала» ~1.5 с ---
function createTV(rng) {
    ...
    function drawChannel(i) {
        ...
        if (mode === 0) {
            ...
            g.addColorStop(0, '#aee1f2'); g.addColorStop(1, '#97c1a9');
            ctx.fillStyle = g; ctx.fillRect(0, 0, 160, 96);
            ctx.fillStyle = '#f7e08b';
            ctx.beginPath(); ctx.arc(120, 26, 14, 0, Math.PI * 2); ctx.fill();
        } else if (mode === 2) {
            ctx.fillStyle = '#20242c'; ctx.fillRect(0, 0, 160, 96);
            for (let k = 0; k < 220; k++) {
            ...
            ctx.fillStyle = palette[i % palette.length]; ctx.fillRect(0, 0, 160, 96);
            ctx.fillStyle = 'rgba(255,255,255,0.85)'; ctx.fillRect(30, 38, 100, 20);
        }
    ...
    return {
        ...
        update(dt) {
            timer += dt;
            if (timer >= 1.5) { timer = 0; channel += 1; drawChannel(channel); }
        }
    };
}
...
function createCat(bounds, rng) {
    ...
    body.rotation.z = Math.PI / 2; body.position.y = 0.14; body.castShadow = true;
    group.add(body);

    const head = new THREE.Mesh(new THREE.SphereGeometry(0.08, 12, 10), fur);
    head.position.set(0.16, 0.2, 0); group.add(head);

    const earGeo = new THREE.ConeGeometry(0.03, 0.06, 6);
    const ear1 = new THREE.Mesh(earGeo, fur); ear1.position.set(0.15, 0.28, 0.035); group.add(ear1);
    const ear2 = new THREE.Mesh(earGeo, fur); ear2.position.set(0.15, 0.28, -0.035); group.add(ear2);

    const tail = new THREE.Mesh(new THREE.CylinderGeometry(0.015, 0.02, 0.28, 6), fur);
    tail.position.set(-0.22, 0.2, 0); tail.rotation.z = Math.PI / 2.5; group.add(tail);

    const curve = makePatrolCurve(bounds, rng);
    const len = curve.getLength();
    const SPEED = 0.35, WALK_T = 6, SIT_T = 2.5;
    let s = rng.range(0, 1), mode = 'walk', modeT = 0;
...
function createDog(bounds, rng) {
    ...
    head.position.set(0.22, 0.3, 0); group.add(head);

    const earGeo = new THREE.BoxGeometry(0.03, 0.08, 0.05);
    const ear1 = new THREE.Mesh(earGeo, std(0x5d4f3f, 0.9)); ear1.position.set(0.2, 0.38, 0.05); group.add(ear1);
    const ear2 = ear1.clone(); ear2.position.z = -0.05; group.add(ear2);

    const tail = new THREE.Mesh(new THREE.CylinderGeometry(0.015, 0.02, 0.18, 6), fur);
    tail.position.set(-0.2, 0.28, 0); tail.rotation.z = -Math.PI / 4; group.add(tail);

    const legGeo = new THREE.BoxGeometry(0.05, 0.14, 0.05);
    for (const [lx, lz] of [[0.12, 0.06], [0.12, -0.06], [-0.12, 0.06], [-0.12, -0.06]]) {
    ...
    return {
        ...
        update(dt, t) {
            ...
            group.scale.y = 1 + Math.sin(t * 3) * 0.02;
        }
    };
}

// --- Торшер: стойка + абажур + тёплый PointLight ---
function createFloorLamp() {
    ...
    pole.position.y = 0.715; group.add(pole);

    const shade = new THREE.Mesh(
        new THREE.CylinderGeometry(0.16, 0.22, 0.3, 16, 1, true),
        new THREE.MeshStandardMaterial({
            color: 0xf2e2c4, roughness: 0.9, side: THREE.DoubleSide,
            emissive: 0xffd9a0, emissiveIntensity: 0.25
        })
    );
    ...
    bulb.position.y = 1.42; group.add(bulb);
    ...
    return {
        group,
        update(dt, t) { light.intensity = 6 + Math.sin(t * 7.3) * 0.15; }
    };
}
...
function createPlant(rng) {
    ...
    pot.position.y = 0.11; pot.castShadow = true; group.add(pot);
...
/**
 * Собирает набор живых пропов из task.anims.
 * @param {string[]} anims
    ...
    for (const kind of anims) {
        ...
        switch (kind) {
            ...
            case 'floorlamp': {
                ...
                if (c) { prop = createFloorLamp(); prop.group.position.set(c[0], 0, c[1]); }
                break;
            }
            case 'plant': {
                const c = freeCorners.pop();
                if (c) { prop = createPlant(rng); prop.group.position.set(c[0], 0, c[1]); }
                break;
            }
            case 'cat': prop = createCat(bounds, rng); break;
            case 'dog': prop = createDog(bounds, rng); break;
            default: prop = null;
        }
        if (prop) { group.add(prop.group); updaters.push(prop.update); }
    }
